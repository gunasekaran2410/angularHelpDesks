...
fixed_report(fixed: TemplateRef<any>, data:any) {
  ...
  console.log(data, "fixed reports 1");

  var submit_file ={
    issueTypeID: parseInt(data.issueTypeID),
    id: parseInt(data.issueItemID)
  }


  this.service.replaceItemValue(submit_file).subscribe((res_value_item: any) => {
    this.replaceItems = res_value_item

    console.log(this.replaceItems, " replaceItems DATA");

  });

  if(data.isItemReplaced == true){
    this.replace_hide = true;
  }
  else{
    this.replace_hide=false;
  ...
  console.log(submit_id, "check DATA")
    ...
    this.service.getItemValue(submit_id).subscribe((res_value: any) => {
      ...
      console.log(this.itemdetailsData, "issue item details")
...
uploadnewfile(input: any) {
  ...
  this.service.InsertFixedImage(formdatafile).subscribe((upload_res: any) => {

    console.log(upload_res, "insert rerutn value");

  this.filedataPath = upload_res

  this.usertype_id = localStorage.getItem('userTypeID');

  var IssueDetail ={
    Id:this.fixed_date_id
  }



       this.service.UpdateFixedImage(this.filedataPath, IssueDetail).subscribe((upload_res1: any) => {

        console.log(upload_res1, "UpdateFixedImage");
